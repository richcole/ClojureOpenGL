...
public class ShapeRenderer {
  ...
  public static Vector[] CUBE_VERTEXES = {
    Vector.M1.plus(Vector.M2).plus(Vector.M3), // 0
    Vector.U1.plus(Vector.M2).plus(Vector.M3), // 1
    Vector.U1.plus(Vector.U2).plus(Vector.M3), // 2
    Vector.M1.plus(Vector.U2).plus(Vector.M3), // 3
    Vector.M1.plus(Vector.M2).plus(Vector.U3), // 4
    Vector.U1.plus(Vector.M2).plus(Vector.U3), // 5
    Vector.U1.plus(Vector.U2).plus(Vector.U3), // 6
    Vector.M1.plus(Vector.U2).plus(Vector.U3)  // 7
  };
  ...
  static final byte TRI_TABLE[][]      = { {}, { 0, 8, 3 }, { 0, 1, 9 }, { 1, 8, 3, 9, 8, 1 }, { 1, 2, 10 },
    ...
    { 1, 8, 3, 1, 9, 8, 5, 10, 6 }, { 1, 6, 5, 2, 6, 1 }, { 1, 6, 5, 1, 2, 6, 3, 0, 8 }, { 9, 6, 5, 9, 0, 6, 0, 2, 6 },
    ...
    { 1, 3, 6, 1, 6, 10, 3, 8, 6, 5, 6, 9, 8, 9, 6 }, { 10, 1, 0, 10, 0, 6, 9, 5, 0, 5, 6, 0 }, { 0, 3, 8, 5, 6, 10 },
    { 10, 5, 6 }, { 11, 5, 10, 7, 5, 11 }, { 11, 5, 10, 11, 7, 5, 8, 3, 0 }, { 5, 11, 7, 5, 10, 11, 1, 9, 0 },
  ...
  public int getCubeIndex(Vector center, double radius, Vector[] ps, double[] ds) {
    int cubeIndex = 0;
    for (int i = 0; i < CUBE_VERTEXES.length; ++i) {
      ps[i] = center.plus(CUBE_VERTEXES[i].times(radius));
      if (Funs.isPos(ds[i])) {
  ...
  public void renderTriangles(int cubeIndex, Vector[] ps, double[] ds, Vector[] vs, Transform tr, VertexCloud cloud, double minRadius, Vector center, double radius) {
    ...
    if (edgeMask != 0) {
      for (int i = 0; i < 12; ++i) {
        if ((edgeMask & (0x1 << i)) > 0) {
          ...
          Vector p1 = ps[e1];
          Vector p2 = ps[e2];
          vs[i] = Funs.midPoint(ds[e1], ds[e2], p1, p2);
        }
      }
      ...
      if ( triIndexes.length > 0 ) {
        for (int i = 0; i < triIndexes.length; i++) {
          ...
          Vector tv = sv.minus(center.minus(Vector.ONES.times(radius))).times(1/(2*radius));
          cloud.addVertex(tr.transform(sv), Vector.Z, tv);
        }
      }
    }
